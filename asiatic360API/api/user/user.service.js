...
module.exports = {
    createUser: (data, callBack) => {
        pool.query(
            "SELECT * FROM users WHERE emp_id = ?", [data.emp_id],
    ...
    getUserbyempid: (emp_id, callBack) => {
        pool.query(
            ...
            (error, results, fields) => {
                ...
                pool.query(
                    "SELECT * FROM users_details WHERE emp_id = '" + results[0].emp_id + "'", [results[0].id],
                    (error, results, fields) => {
                        if (error) {
                            return callBack(error);
                        }
    ...
    loginUserbyempid: (emp_id, emp_password, callBack) => {
        pool.query(
            "SELECT emp_password FROM users WHERE emp_id = ?", [emp_id],
            (error, results, fields) => {
                if (error) {
                    return callBack(error);
                } else if (results[0] != null) {
